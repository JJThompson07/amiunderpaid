...
- [ ] 2.2 Update `server/api/market-data/salary.ts` to remove the hardcoded `-10` suffix on `jobsCacheKey` (`salary.ts:319`) and align it with the harmonized `jobs.ts` cache key, update `server/api/market-data/tests/salary.spec.ts`, and verify with `pnpm vitest run server/api/market-data/tests/salary.spec.ts`.

## 3. Frontend Composables & Search Form Mode

- [ ] 3.1 Update `app/composables/useJobs.ts` to expose `similarJobsData` / `similarResults` and full listing arrays, update `app/composables/tests/useJobs.spec.ts`, and verify with `pnpm vitest run app/composables/tests/useJobs.spec.ts`.
- [ ] 3.2 Implement `app/composables/useJobSearchEngine.ts` with route param parsing, multi-criteria sorting (`relevance`, `salary_max`, `salary_avg`, `salary_min`), dual-tier list separation, write unit tests in `app/composables/tests/useJobSearchEngine.spec.ts`, and verify with `pnpm vitest run app/composables/tests/useJobSearchEngine.spec.ts`.
- [ ] 3.3 Extend `app/components/BaseSearchForm.vue` to support `mode="jobs"`, hiding the salary input, customizing button text, and routing to `/jobs/...`.

## 4. Job Search Pages, Components & SEO

- [ ] 4.1 Add localized strings across `i18n/locales/en-GB/*.json` and `i18n/locales/en-US/*.json` (`navbar.json`, `buttons.json`, `search.json`, `meta.json`, `card.json`, `sections.json`) for all job search UI labels and meta tags.
- [ ] 4.2 Create `app/pages/jobs/index.vue` landing page with hero header, `BaseSearchForm` (`mode="jobs"`), popular role tags, and value proposition cards.
- [ ] 4.3 Create `app/pages/jobs/[title]/[country]/[[location]].vue` dynamic results page with breadcrumbs, prefilled search bar, salary benchmark pill link, sorting toolbar, separate Exact Matches and Similar Roles sections (`AmICardRole`), recruiter card integration, `LazySectionNoData`, and JSON-LD structured data.
- [ ] 4.4 Update `app/components/AmI/NavBar.vue` to add the "Jobs" navigation link for all visitor roles.
- [ ] 4.5 Update `server/routes/sitemap.xml.ts` and `server/routes/tests/sitemap.xml.spec.ts` to index `/jobs` and dynamic `/jobs/...` routes.

## 5. End-to-End Testing & Verification Gate

- [ ] 5.1 Create Playwright e2e test `e2e/job-search.spec.ts` testing landing page search submission, sorting by salary, dual-tier listing display, and navigation to salary benchmark page.
- [ ] 5.2 Run complete local verification gate `pnpm test:verify` ensuring zero lint, typecheck, coverage (<80%), e2e, or rules test failures.
