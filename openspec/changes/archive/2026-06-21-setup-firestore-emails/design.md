...
- **SMTP credential exposure in configuration**
  - _Mitigation_: SMTP password and username will be stored securely using Google Cloud Secret Manager (default behavior for Firebase Extensions).
- **Spam flags on candidate auto-replies**
  - _Mitigation_: Ensure the `from` address uses a verified domain with valid SPF/DKIM/DMARC records.
