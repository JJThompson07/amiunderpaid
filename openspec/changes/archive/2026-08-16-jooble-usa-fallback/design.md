...
- **Risk: Jooble salary data is empty.** → _Mitigation_: The `MarketJob` interface accommodates undefined `minimumSalary` and `maximumSalary`. If computing histograms and no jobs have salaries, the API will return a 404 or empty histogram to avoid displaying skewed data on the client.
- **Risk: Rate limits on Jooble API.** → _Mitigation_: Fallback provider is only hit if Adzuna fails. Given this is secondary, volume is naturally lower. If Jooble also rate limits, we will propagate a 503 back to the client.
