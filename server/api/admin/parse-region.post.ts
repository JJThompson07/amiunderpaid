...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (country === 'UK') {
      ...
      // Percentile Columns safely checked
      const pct10Idx = headers.findIndex((h) => h != null && String(h).trim() === '10');
      const pct25Idx = headers.findIndex((h) => h != null && String(h).trim() === '25');
      const pct75Idx = headers.findIndex((h) => h != null && String(h).trim() === '75');
      ...
      for (let i = headerRowIndex + 1; i < rawData.length; i++) {
        ...
        if (!row || row[locIdx] == null || String(row[locIdx]).trim() === '') {
          continue;
        }
      ...
      const titleIdx = headers.findIndex(
        (h) => h != null && String(h).toUpperCase() === 'OCC_TITLE'
      );
      const salaryIdx = headers.findIndex(
        (h) => h != null && String(h).toUpperCase() === 'A_MEDIAN'
      );
      const meanIdx = headers.findIndex((h) => h != null && String(h).toUpperCase() === 'A_MEAN');
      const codeIdx = headers.findIndex((h) => h != null && String(h).toUpperCase() === 'OCC_CODE');
      const locIdx = headers.findIndex(
        (h) => h != null && String(h).toUpperCase() === 'AREA_TITLE'
      );

      // Percentile Columns safely checked
      const pct10Idx = headers.findIndex((h) => h != null && String(h).toUpperCase() === 'A_PCT10');
      const pct25Idx = headers.findIndex((h) => h != null && String(h).toUpperCase() === 'A_PCT25');
      const pct75Idx = headers.findIndex((h) => h != null && String(h).toUpperCase() === 'A_PCT75');
      const pct90Idx = headers.findIndex((h) => h != null && String(h).toUpperCase() === 'A_PCT90');
