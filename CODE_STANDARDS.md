...
- **Dynamic/runtime colors without a predefined theme scale:** If a component needs a Tailwind-style 50-900 tonal scale (light-tint background + matching dark text, e.g. a badge) for a color that has no static entry in `app/assets/css/main.css` — most commonly a runtime-generated categorical palette, like one series per item in a data visualization — do **not** hand-author a new static scale in `main.css`. Call `generateColorScale(hex)` from `shared/utils/color.ts`, which derives a full `{50, 100, ..., 900}` scale from a single seed hex (hue + saturation preserved, lightness re-derived per stop from a fixed ramp) and apply the resulting shades via inline `:style` bindings. See `app/components/Section/Shared/IndustryTrendsChart.vue` for the reference implementation (chart line uses the `500` stop, its legend pill uses `100`/`800` for background/text — the same light-bg/dark-text pattern used by every other badge in this app, just computed instead of static). Also note: colors intended only for JS/`getComputedStyle()` consumption (never used as a literal Tailwind utility class like `bg-foo-500`) must be declared as plain `:root` custom properties, **not** inside the `@theme` block — Tailwind v4 tree-shakes `@theme` tokens that no scanned template references as an actual utility class, so an unreferenced `@theme` color silently never reaches the compiled CSS.
...
- **Reusable `AmI/**` components with real logic:** A base component under `app/components/AmI/**` that carries genuine logic (state derivation, clamping, formatting — not a thin presentational wrapper) gets a unit test under an adjacent `tests/` dir too, same convention as composables/server utils (e.g. `app/components/AmI/Input/tests/RangeSlider.spec.ts`). Use `@vue/test-utils` (`mount`), and stub Nuxt's auto-imported composables/Vue Composition API globals the component references (`ref`, `computed`, `useI18n`, etc.) via `vi.stubGlobal`, mirroring the pattern already used to test composables (see `app/composables/tests/useIndustryTrends.spec.ts`) — there is no Nuxt-aware test environment configured in `vitest.config.ts`, so these are not auto-injected under plain Vitest the way they are inside the real app. Note: `vitest.config.ts`'s `coverage.include` glob does not currently cover `app/components/**` at all, so the 80%-per-file threshold below does not mechanically enforce this convention yet — write the test anyway; do not expand `coverage.include` to a blanket `app/components/**` as a side effect of a single new component, since that would retroactively demand coverage from every pre-existing, currently-untested component in the repo (a separate, much larger change).
