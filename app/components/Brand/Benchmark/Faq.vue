    ...
    <div class="flex flex-col gap-10">
      ...
      <div v-for="section in sections" :key="section.id">
        <h3 class="text-lg sm:text-2xl font-bold text-slate-900 mb-4 flex items-center gap-3">
          ...
          {{ t(`faq.benchmark.questions.section.${section.id}.title`) }}
...
const faqSchema = computed(() => {
  ...
  sections.value.forEach((section) => {
    section.keys.forEach((key) => {
      mainEntity.push({
        '@type': 'Question',
        name: t(`faq.benchmark.questions.section.${section.id}.${key}.question`),
        acceptedAnswer: {
          '@type': 'Answer',
          text: t(`faq.benchmark.questions.section.${section.id}.${key}.answer`)
        }
      });
    });
  });
