...
test.describe('SSR Tenant Rendering', () => {
  ...
  test('Am I Underpaid UK renders UK specific initial HTML without hydration', async ({ page }) => {
    ...
    const response = await page.goto('http://ami-uk.localhost:3000/salary/software-engineer/uk', { waitUntil: 'domcontentloaded' });
  ...
  test('Am I Underpaid USA renders USA specific initial HTML without hydration', async ({ page }) => {
    // Use the ami-us.localhost domain to trigger the US tenant middleware
    const response = await page.goto('http://ami-us.localhost:3000/salary/software-engineer/usa', { waitUntil: 'domcontentloaded' });
